...
public class ToastBuilder : MonoBehaviour
{
    ...
    [SerializeField] RTLTextMeshPro ToastTXT;
    ToastOBJ _ToastOBJ;

    List<ToastOBJ> toastOBJs = new List<ToastOBJ>();
    const int FirstToastsCount = 10;

    public void SendToast(string toast, bool isPositive = true)
    {
        ToastTXT.text = toast;
        _ToastBack.color = isPositive ? _Color.G_ToastGreen : _Color.R_ToastRed;
        gameObject.SetActive(true);
        Timers.Instance.StartTimer(1f, () => gameObject.SetActive(false));
    }
